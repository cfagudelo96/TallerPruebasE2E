...
describe('Los estudiantes login exitoso', function() {
    it('Visita los estudiantes, crea una cuenta y hace login', function() {
        ...
        cy.contains('Ingresar').click()
        var fecha = new Date()
        registrarUsuario(fecha.getTime())
        cy.contains('Registro exitoso!')
        cy.contains('Ok').click()
        cy.get('.fa-user-circle').click()
        cy.get('.dropdown-menu').contains('Salir').click()
        cy.contains('Ingresar').click()
        cy.get('.cajaLogIn').find('input[name="correo"]').click().type("pepebot" + fecha.getTime() + "@test.com")
        cy.get('.cajaLogIn').find('input[name="password"]').click().type(fecha.getTime())
        cy.get('.cajaLogIn').contains('Ingresar').click()
    })
})
